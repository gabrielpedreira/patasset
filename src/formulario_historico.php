<?php
/*
 * formulario_historico.php
 * Gera o Termo de Movimentação a partir de registros da tabela `historico`.
 */
session_start();
require 'conexao.php';
...
$sql = "
SELECT
    id, `data`, descricao, marca, modelo, serie, tag,
    unidade, setor,
    unidade_dest, setor_dest,
    obs_mov, tipo_mov
FROM historico
...
/* ===============================
   DADOS COMUNS (do primeiro item)
================================ */
$primeiro    = $itens[0];
$unidadeOrig = $primeiro['unidade'];
$setorOrig   = $primeiro['setor'];
$unidadeDest = $primeiro['unidade_dest'];
$setorDest   = $primeiro['setor_dest'];
$obsMov      = $primeiro['obs_mov'];
$tipo_mov    = $primeiro['tipo_mov'] ?: (($unidadeOrig !== $unidadeDest) ? "Transferência entre Unidades" : "Empréstimo entre Setores");

$data = $primeiro['data']
    ? date('d/m/Y', strtotime($primeiro['data']))
    : date('d/m/Y');
...
.form{background:#fff;max-width:950px;margin:auto;padding:40px;border:1px solid #000;}
...
input,textarea,select{
    ...
    padding:4px;font-size:14px;outline:none;box-sizing:border-box;
}
...
.tbl-itens{width:100%;border-collapse:collapse;font-size:13px;margin:10px 0 20px;}
.tbl-itens th{background:#1d4ed8;color:#fff;padding:7px 8px;text-align:left;}
.tbl-itens td{padding:6px 8px;border:1px solid #cbd5e1;}
...
.linha-ass{border-top:1px solid #000;text-align:center;padding-top:6px;font-size:13px;}

.btn-print{margin-top:30px;text-align:center;}

button{
    padding:10px 25px;border:none;background:#2563eb;
...
.painel-email{
    ...
    padding:15px;margin:20px 0;background:#eff6ff;
}
.painel-email h3{margin:0 0 12px;color:#1d4ed8;font-size:15px;}

.checkline-email{display:flex;align-items:center;gap:8px;margin-bottom:12px;font-weight:600;font-size:14px;}

.campo-manual{display:none;margin-top:10px;}
.campo-manual input{border:none;border-bottom:1px solid #000;width:100%;padding:4px;font-size:14px;}
...
const todoItens = <?= json_encode(array_map(function($item) {
    return [
        ...
        'setor'           => $item['setor']         ?? '',
        'unidade_destino' => $item['unidade_dest']  ?? '',
        'setor_destino'   => $item['setor_dest']    ?? '',
    ];
...
function validarImpressao(){
    ...
    if (semEmail) {
        ...
        if (!nomeManual) { alert("Digite o nome do responsável que recebe."); return; }
        window.print();
    ...
    if (!idDest) { alert("Selecione o responsável que recebe."); return; }
...
function bloquear(){ document.body.style.opacity='0.7'; document.body.style.pointerEvents='none'; }
function desbloquear(){ document.body.style.opacity='1'; document.body.style.pointerEvents='auto'; }
</script>
